...
export default function GiftPanel({ gifts }: GiftPanelProps) {
  ...
  return (
    <div className="bg-gray-800 border-2 border-gray-700 rounded-2xl shadow-2xl p-6 h-[600px] flex flex-col">
      <div className="mb-4">
        <h2 className="text-2xl font-bold text-yellow-400 pb-3 border-b-2 border-yellow-500/30 flex items-center gap-2">
          🎁 Gifts
        </h2>

        {/* Toplam Değer */}
        {totalDiamonds > 0 && (
          <div className="mt-3 bg-gradient-to-r from-yellow-900/30 to-amber-900/30 rounded-lg p-3 border-2 border-yellow-500/50">
            <div className="text-sm text-gray-400 mb-1">Total Gift Value:</div>
        ...
        {gifts.length === 0 ? (
          ...
          <AnimatePresence initial={false}>
            {gifts.map((gift, index) => {
              ...
              const uniqueKey = `${gift.username}-${gift.timestamp}-${gift.giftId}-${gift.repeatCount}`
              ...
              return (
                ...
                >
                  ...
                  {totalDiamond >= 1000 && (
                    <div className="absolute top-0 right-0 bg-yellow-400 text-gray-900 px-3 py-1 rounded-bl-lg font-bold text-xs shadow-lg">
                      🔥 HIGH VALUE!
                    </div>
                  )}
